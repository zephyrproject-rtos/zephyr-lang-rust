...
mod async_io {
    /// Per-controller static data for GPIO (stub when async-drivers is disabled).
    pub struct GpioStatic;
...
impl GpioPin {
    ...
    /// # Safety
    ///
    /// - `device` must be a valid pointer to a Zephyr GPIO controller device
    ///   that remains valid for the lifetime of the returned `GpioPin`.
    /// - `device_static` must be the `GpioStatic` associated with that same
    ///   controller.
    /// - `pin` and `dt_flags` must fit within Zephyr's `gpio_pin_t` and
    ///   `gpio_dt_flags_t` types respectively (values are truncated on cast).
